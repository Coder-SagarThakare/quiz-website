...
function Question({ question, currentQuestionNo, setCurrentQuestionNo, questionCount }) {
        ...
        <div className='border border-danger h-100 p-2 d-flex flex-column justify-content-between'>
            <div className="p-5 glass-effect d-flex flex-column align-items-start justify-content-between user-select-none">
                ...
                <div className="d-flex  justify-content-center w-100 gap-5">
                    ...
                    <Button title={"Prev"} onClick={() => setCurrentQuestionNo(--currentQuestionNo)} className={`bg-primary ${currentQuestionNo === 0 && "disabled"}`} />
                    <Button title={"Next"} onClick={() => setCurrentQuestionNo(++currentQuestionNo)} className={`bg-primary ${currentQuestionNo === questionCount-1 && "disabled"}`} />
                </div>
            </div>
            ...
            <div className='glass-effect p-2 d-flex justify-content-center gap-2'>
                ...
                {Array.from({ length: questionCount }, (_, i) => (
                    <Button
                        key={i}
                        title={i + 1}
                        onClick={() => setCurrentQuestionNo(i)} className={`text-light ${currentQuestionNo === i && 'bg-primary'}`}
                    />
                ))}
            </div>
        </div>
